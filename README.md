...
Side-by-side detection comparisons between D3F-Det and the baseline YOLOv12-s on real UAV-captured images from all three datasets are shown below.
...
Ensure the weight files are placed in the `runs/` directory with the correct filenames listed in Section 4.
